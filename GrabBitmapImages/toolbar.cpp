...
HRESULT Toolbar::Create(HINSTANCE hInstance, HWND hParent, DWORD_PTR id, DWORD dwStyle)
{
	...
	/* This message is sent to the toolbar, before sending the TB_ADDBITMAP and TB_ADDBUTTONS  message  */
...
//				This function sends message to Winddow or Windows.
//				It calls Window Procedure for the specified window and does not return 
//				untill the window procedure has processed the message.
//--------------------------------------------------------------------------------------
...
HRESULT Toolbar::ShowToolTip(NMTTDISPINFO *pDispInfo)
{
    ...
    /* Check if the NMTTDISPINFO structure contains a button ID.(It might contain nothing, or an HWND.) */
...
// mask: Color mask
//--------------------------------------------------------------------------------------
HRESULT Toolbar::SetImageList(ButtonState state,UINT nBitmapID,const Size& buttonSize,DWORD numButtons,COLORREF mask)
{
	/*
...
{
    assert(hBand!=NULL);  /* Check that there is that child window is existed */
    assert(m_hwnd!=NULL); /* Check that there is main window is existed */
    ...
    rbBand.fMask =RBBIM_CHILD | RBBIM_CHILDSIZE | RBBIM_STYLE | RBBIM_ID | RBBIM_SIZE | RBBIM_IDEALSIZE |RBBIM_BACKGROUND;
	...
	if(nBtnCount>0){
		...
		/* no buttons, either not a toolbar or really has no buttons */
...
{
    UINT position;
    HRESULT hResult=BandIdToIndex(id, &position);
    if(FAILED(hResult))
        return hResult;
    BOOL bResult=(BOOL)SendMessage(RB_SHOWBAND, position, bShow);
    if(bResult)
		return S_OK;
    else
        return E_FAIL;
